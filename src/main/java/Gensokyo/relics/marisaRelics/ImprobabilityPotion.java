package Gensokyo.relics.marisaRelics;
...
public class ImprobabilityPotion extends CustomRelic implements ClickableRelic {
    ...
    @Override
    public void obtain() {
        if (AbstractDungeon.player.hasRelic(IngredientList.ID)) {
            int relicIndex = -1;
            for (int i = 0; i < AbstractDungeon.player.relics.size(); i++) {
                if (AbstractDungeon.player.relics.get(i) instanceof IngredientList) {
                    relicIndex = i;
                    break;
                }
            }
