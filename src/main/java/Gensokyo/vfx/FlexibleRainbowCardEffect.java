...
public class FlexibleRainbowCardEffect extends AbstractGameEffect {
    float x;
    float y;
    private AtlasRegion img;

    public FlexibleRainbowCardEffect(AbstractCreature target) {
        if (this.img == null) {
            this.img = ImageMaster.CRYSTAL_IMPACT;
        }

        this.x = target.hb.cX - (float)this.img.packedWidth / 2.0F;
        this.y = target.hb.cY - (float)this.img.packedHeight / 2.0F;
